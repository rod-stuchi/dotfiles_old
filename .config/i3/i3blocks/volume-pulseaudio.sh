#!/bin/bash
# https://github.com/vivien/i3blocks-contrib/blob/master/volume-pulseaudio/volume-pulseaudio
...
case "$BLOCK_BUTTON" in
    ...
    3) pavucontrol ;;
    4) pulseaudio-ctl up $AUDIO_INTERVAL ;;
    5) pulseaudio-ctl down $AUDIO_INTERVAL ;;
esac
...
while getopts F:f:pH:M:L:T:t:C:c:h opt; do
    case "$opt" in
        F) LONG_FORMAT="$OPTARG" ;;
        f) SHORT_FORMAT="$OPTARG" ;;
        p) USE_PERCENT=0 ;;
...
case "$BLOCK_BUTTON" in
    ...
    2) pulseaudio-ctl mute ;;
    3) pavucontrol ;;
    4) pulseaudio-ctl up 10 ;;
    5) pulseaudio-ctl down 10;;
esac
