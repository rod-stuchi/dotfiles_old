...
for d in $(ls -d */ | sed 's|/$||'); do
  ...
  # R => release
  read -r S E P R <<< $(echo $d | awk 'match($0, /([sS][0-9]+).*([eE][0-9]+).*(webrip|WEBRip|408p|720p|1080p).*-(\w+)(|\[\w+\])$/, a) { print a[1]" "a[2]" "a[3]" "a[4] }')
  ...
  if [[ ! -z $S ]]; then
    ...
    if [[ ! -z $zfile ]]; then
      ...
      case $ext in
        zip)
          subfile=$(7z l $zfile | awk '/srt$/ { print $NF }' | rg "$S.*$E.*$P.*$R" | head -n1)
          printf " > 📂%s%s %6s %-8s - 🗄 %s \n" $S $E $P $R $(basename $subfile)
          7z -y e $zfile "$subfile" >/dev/null
          subfile=$(basename $subfile)
          ;;
        rar)
          subfile=$(unrar l $zfile | awk '/srt$/ { print $NF }' | rg "$S.*$E.*$P.*$R" | head -n1)
          printf " > 📂%s%s %6s %-8s - 🗄 %s \n" $S $E $P $R $(basename $subfile)
